...
function datagram:new (p, class, options)
   ...
   if not o._recycled then
      ...
      o._packet = ffi.new("struct rte_mbuf *[1]")
...
function datagram:parse_match (class, check)
   ...
   local proto = class:new_from_mem(self._packet[0]:getBytes() + parse.offset,
                                    self._packet[0]:getSize() - parse.offset)
...
function datagram:payload (mem, size)
   ...
   return self._packet[0]:getBytes() + self._parse.offset,
          self._packet[0]:getSize() - self._parse.offset
end

-- Return the location and size of the entire packet buffer
function datagram:data ()
   local p = self._packet
   return p[0]:getBytes(), p[0]:getSize()
end
...
function datagram:commit ()
   if self._delayed_commit then
      ...
      local push = self._push
      C.memmove(p:getBytes() + push.size, p:getBytes() + offset, p:getSize() - offset)
      ffi.copy(p:getBytes(), push.buffer + push.offset, push.size)
      local diff = offset - push.size
      p:setSize(p:getSize() - diff)
